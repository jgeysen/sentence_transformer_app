...
def train_function(index, args, queue):
    ...
    for global_step in tqdm.trange(args.steps, disable=not xm.is_master_ordinal()):
        #### Get the batch data
        batch = queue.get()
        # print(index, "batch {}x{}".format(len(batch), ",".join([str(len(b)) for b
        # in batch])))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=20,
        help="Use 20 for cossim, and 1 when you work with unnormalized embeddings "
        "with dot product",
    )
    parser.add_argument(
        "--no_normalize",
    ...
    print("Training done")
    print(
        "It might be that not all processes exit automatically. In that case you must "
...
# Script was called via:
# python train_many_data_files_v2.py --steps 200000 --batch_size 80
# --batch_size_triplets 40 --model microsoft/mpnet-base --max_length_a 64
# --max_length_b 250 --scale 1 --pooling cls
# --no_normalize train_data_configs/multi-qa_v1.json
# output/multi-qa_v1-mpnet-base-cls_dot
